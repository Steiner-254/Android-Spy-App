...
class StreamScreen(private val ctx: Context, private val handler: Handler, private val resultCode: Int, private val data: Intent?) : Thread() {
    ...
    private fun takeScreenShot() {
        ...
        val imageReader: ImageReader = ImageReader.newInstance(metrics.widthPixels, metrics.heightPixels, PixelFormat.RGBA_8888, 2)

        val onImageAvailableListener = ImageReader.onImageAvailableListener {
            it: ImageReader? -> SystemClock.sleep(100)
        ...
        mMediaProjection.createVirtualDisplay("ScreenCapture", metrics.widthPixels, metrics.heightPixels, metrics.densityDpi, DisplayManager.VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR, imgReader.surface, null, handler)
